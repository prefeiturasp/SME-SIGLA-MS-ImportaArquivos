...
class EscolaParaVagaSerializer(serializers.Serializer):
    ...
    codigo_integracao = serializers.CharField(trim_whitespace=True, required=False, allow_blank=True, allow_null=True, default='')
...
class VagaInputSerializer(serializers.Serializer):
    ...
    """
    vagas_definitivas = serializers.IntegerField(required=False, default=0, min_value=0)
    vagas_precarias = serializers.IntegerField(required=False, default=0, min_value=0)
...
class VagasPayloadSerializer(serializers.Serializer):
    ...
    def validate_vagas(self, value: Any) -> Any:
        ...
        if not value:
            raise serializers.ValidationError('Pelo menos uma vaga deve ser enviada.')
        return value
...
class ExportacaoVagasSigpecCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['processo_uuid', 'cargo_uuid', 'concurso_uuid', 'concurso_nome', 'processo_nome', 'cargo_nome', 'cargo_codigo']
...
class ExportacaoVagasSigpecListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        """Define Meta."""
        model = ExportacaoVagasSigpec
        fields = ['uuid', 'criado_em', 'atualizado_em', 'processo_uuid', 'cargo_uuid', 'concurso_uuid', 'concurso_nome', 'processo_nome', 'cargo_nome', 'cargo_codigo']
        read_only_fields = ['uuid', 'criado_em', 'atualizado_em', 'processo_uuid', 'cargo_uuid', 'concurso_uuid', 'concurso_nome', 'processo_nome', 'cargo_nome', 'cargo_codigo']
