...
class TestExportacaoLoteCreate:
    ...
    def test_sucesso_persiste_status_sucesso_e_conteudo(self, api_client, payload_valido):
        ...
        assert registro.conteudo_arquivo == conteudo
    ...
    def test_incompleto_persiste_status_atencao(self, api_client, payload_valido):
        ...
        registro = ExportacaoLote.objects.order_by("-criado_em").first()
        assert registro.status == "ATENCAO"
    ...
    def test_serializer_invalido_retorna_400(self, api_client):
...
class TestExportacaoLoteDownload:
    ...
    def test_uuid_inexistente_retorna_404(self, api_client):
...
class TestExportacaoLoteRetrieve:
    ...
    def test_retrieve_uuid_inexistente_retorna_404(self, api_client):
        response = api_client.get(f"{LOTE_URL}{_uuid()}/")
        assert response.status_code == 404


# ─── _gerar_conteudo_erro ───────────────────────────────────────────────────
...
class TestGerarConteudoErro:
    ...
    def test_usa_lote_uuid_quando_numero_lote_none(self):
        lote_uuid = uuid.uuid4()
        instance = ExportacaoLote(numero_lote=None, lote_uuid=lote_uuid)
...
class TestCabecalhoExportacaoLoteViewSet:
    ...
    def test_list_vazia_retorna_200(self, api_client):
        response = api_client.get(CABECALHO_URL)
        assert response.status_code == 200

    def test_create_retorna_201_com_campos(self, api_client):
        payload = {
    ...
    def test_partial_update_altera_campo(self, api_client):
    ...
    def test_filtro_ativo_false_retorna_apenas_inativos(self, api_client):
