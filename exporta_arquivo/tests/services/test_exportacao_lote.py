...
from exporta_arquivo.services.exportacao_lote import _data_para_ddmmyyyy, exportar_lote, gerar_conteudo_lote
...
def _candidato(cand_uuid: Any=None, numero_lote: Any=1, codigo_sigpec: Any='SIG001', chave_inscrito: Any='123', nome: Any='João') -> Any:
    """Executa  candidato.
    
    Args:
        cand_uuid: Parâmetro cand uuid da operação.
        numero_lote: Parâmetro numero lote da operação.
        codigo_sigpec: Parâmetro codigo sigpec da operação.
        chave_inscrito: Parâmetro chave inscrito da operação.
    ...
    return {'uuid': cand_uuid, 'numero_lote': numero_lote, 'codigo_sigpec': codigo_sigpec, 'chave_inscrito': chave_inscrito, 'candidato': {'uuid': cand_uuid, 'nome': nome}}

def _escolha(candidato_uuid: Any, situacao: Any='escolha', codigo_integracao: Any='ESCOLA01') -> Any:
    """Executa  escolha.
    
    Args:
        candidato_uuid: Parâmetro candidato uuid da operação.
        situacao: Parâmetro situacao da operação.
        codigo_integracao: Parâmetro codigo integracao da operação.
    ...
    Raises:
        Nenhuma exceção específica documentada.
    """
    return {'candidato_uuid': candidato_uuid, 'situacao': situacao, 'criado_em': '2024-06-15T10:00:00Z', 'vaga_escola': {'escola': {'codigo_integracao': codigo_integracao}}}
...
def _recusa(candidato_uuid: Any) -> Any:
    ...
    Raises:
        Nenhuma exceção específica documentada.
    """
    return {'candidato_uuid': candidato_uuid, 'situacao': 'recusa', 'criado_em': '2024-06-15T10:00:00Z', 'vaga_escola': None}
...
class TestGerarConteudoLote:
    ...
    def test_data_escolha_formatada_ddmmyyyy(self) -> None:
        ...
        escolha = {'candidato_uuid': cand_uuid, 'situacao': 'escolha', 'criado_em': '2024-08-20T00:00:00Z', 'vaga_escola': {'escola': {'codigo_integracao': 'X'}}}
        out = gerar_conteudo_lote([candidato], {cand_uuid: escolha})
    ...
    def test_varios_candidatos_geram_varias_linhas(self) -> None:
        ...
        """
        c1, c2 = (_uuid(), _uuid())
        candidatos = [_candidato(cand_uuid=c1, numero_lote=1, codigo_sigpec='SIG1', chave_inscrito='1'), _candidato(cand_uuid=c2, numero_lote=1, codigo_sigpec='SIG2', chave_inscrito='2')]
        escolhas = {c1: _escolha(c1, codigo_integracao='A'), c2: _escolha(c2, codigo_integracao='B')}
    ...
    def test_lookup_por_uuid_do_campo_candidato(self) -> None:
        ...
        inner_uuid = _uuid()
        outer_uuid = _uuid()
        candidato = {'uuid': outer_uuid, 'numero_lote': 3, 'codigo_sigpec': 'X', 'chave_inscrito': '9', 'candidato': {'uuid': inner_uuid, 'nome': 'Ana'}}
    ...
    def test_lookup_fallback_por_uuid_externo(self) -> None:
        ...
        """
        outer_uuid = _uuid()
        candidato = {'uuid': outer_uuid, 'numero_lote': 1, 'codigo_sigpec': 'Y', 'chave_inscrito': '8', 'candidato': {}}
...
class TestExportarLote:
    ...
    def instance(self) -> Any:
        ...
        return ExportacaoLote.objects.create(concurso_uuid=uuid.uuid4(), concurso_nome='Concurso Teste', numero_lote=1, lote_uuid=uuid.uuid4())

    def test_sucesso_retorna_conteudo_com_cabecalho(self, instance: Any) -> None:
        ...
        with patch('exporta_arquivo.services.exportacao_lote.ApiCandidatosService') as MockCand, patch('exporta_arquivo.services.exportacao_lote.ApiEscolhasService') as MockEsc:
    ...
    def test_chama_candidatos_com_concurso_uuid_e_numero_lote(self, instance: Any) -> None:
        ...
        MockCand.return_value.get_habilitados.assert_called_once_with(concurso_uuid=str(instance.concurso_uuid), numero_lote=instance.numero_lote)

    def test_chama_escolhas_com_candidato_uuids_e_concurso_uuid(self, instance: Any) -> None:
        ...
        with patch('exporta_arquivo.services.exportacao_lote.ApiCandidatosService') as MockCand, patch('exporta_arquivo.services.exportacao_lote.ApiEscolhasService') as MockEsc:
            ...
            MockEsc.return_value.get_escolhas.return_value = [_escolha(cand_uuid)]
            exportar_lote(instance)
        MockEsc.return_value.get_escolhas.assert_called_once_with(candidato_uuids=[cand_uuid], concurso_uuid=str(instance.concurso_uuid))

    def test_candidato_sem_escolha_levanta_incompleta(self, instance: Any) -> None:
    ...
    def test_candidato_sem_uuid_nao_incluido_na_lista_enviada_a_api_escolhas(self, instance: Any) -> None:
        ...
        """
        cand_uuid = _uuid()
        candidatos = [{'uuid': '', 'concurso_candidato_uuid': '', 'numero_lote': 1, 'codigo_sigpec': 'X', 'chave_inscrito': '1', 'candidato': {}}, _candidato(cand_uuid=cand_uuid)]
        with patch('exporta_arquivo.services.exportacao_lote.ApiCandidatosService') as MockCand, patch('exporta_arquivo.services.exportacao_lote.ApiEscolhasService') as MockEsc:
            MockCand.return_value.get_habilitados.return_value = candidatos
            MockEsc.return_value.get_escolhas.return_value = [_escolha(cand_uuid)]
            try:
    ...
    def test_primeira_escolha_por_candidato_prevalece(self, instance: Any) -> None:
        ...
        cand_uuid = _uuid()
        candidatos = [_candidato(cand_uuid=cand_uuid)]
        escolha_recente = {'candidato_uuid': cand_uuid, 'situacao': 'escolha', 'criado_em': '2024-09-01T00:00:00Z', 'vaga_escola': {'escola': {'codigo_integracao': 'FIRST'}}}
        escolha_antiga = {'candidato_uuid': cand_uuid, 'situacao': 'recusa', 'criado_em': '2024-08-01T00:00:00Z', 'vaga_escola': None}
        with patch('exporta_arquivo.services.exportacao_lote.ApiCandidatosService') as MockCand, patch('exporta_arquivo.services.exportacao_lote.ApiEscolhasService') as MockEsc:
            MockCand.return_value.get_habilitados.return_value = candidatos
