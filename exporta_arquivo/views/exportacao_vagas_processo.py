...
- list: listagem (ou, se processo_uuid e cargo_uuid na query, retorna arquivo .txt — compatibilidade).
...
from ..services.exportacao_vagas_processo import buscar_vagas_escolas, formatar_arquivo_vagas_processo
...
class ExportacaoVagasProcessoViewSet(BaseExportacaoViewSet):
    ...
    - GET list com processo_uuid e cargo_uuid na query: retorna arquivo .txt (comportamento legado).
    """
    ...
    def executar_exportacao(self, instance):
        """Executa a exportação vagas processo, gera o arquivo e persiste conteúdo e nome no registro."""
        ...
        nome_arquivo = f"exportacao-vagas-processo-{cargo_safe}.{instance.cargo_codigo}.{desc_safe}.txt"
