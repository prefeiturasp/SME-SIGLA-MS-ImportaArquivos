...
from ..services.exceptions import ExportacaoBadRequestException, ExportacaoNotFoundException, ExportacaoServiceUnavailableException
...
class BaseExportacaoViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request: Any, *args: Any, **kwargs: Any) -> Any:
        """Listagem paginada ou, se processo_uuid, cargo_uuid e cargo_codigo na.

        query, retorna arquivo (download direto).

        Para download direto são obrigatórios: processo_uuid, cargo_uuid e
        cargo_codigo na query.
        """
        ...
        if processo_uuid and cargo_uuid:
            if not cargo_codigo_raw:
                return Response({'detail': 'Para download direto, envie processo_uuid, cargo_uuid e cargo_codigo na query.'}, status=status.HTTP_400_BAD_REQUEST)
            try:
                cargo_codigo = int(cargo_codigo_raw)
            except (TypeError, ValueError):
                return Response({'detail': 'cargo_codigo na query deve ser numérico.'}, status=status.HTTP_400_BAD_REQUEST)
            return self.gerar_arquivo(processo_uuid, cargo_uuid, concurso_uuid=concurso_uuid, processo_nome=processo_nome, cargo_nome=cargo_nome, cargo_codigo=cargo_codigo)  # type: ignore[call-arg]
        return super().list(request, *args, **kwargs)

    def gerar_arquivo(self, instance: Any) -> None:
        """Gera resposta de arquivo para os UUIDs dados.

        processo_nome e cargo_nome opcionais (vindos do front no create, do
        registro no download,
        ou da query no GET list). Quando ausentes (ex.: download direto via GET
        list sem create),
        o serviço usa fallback: API de convocação para cargo_nome e "processo"
        para processo_nome.
        Subclasses devem implementar (ex.: chamar serviço de exportação e
        retornar HttpResponse/Response).
        """
    ...
    def create(self, request: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            return Response({'mensagem': exc.mensagem, 'detail': exc.detalhes}, status=status.HTTP_404_NOT_FOUND)
            ...
            return Response({'mensagem': exc.mensagem, 'detail': exc.detalhes}, status=status.HTTP_502_BAD_GATEWAY)
        except Exception as exc:
            logger.exception('Erro interno (500) inesperado na exportação %s: %s', instance.uuid, exc)
            return Response({'mensagem': 'Erro interno ao gerar exportação.', 'detail': str(exc)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        return self.gerar_arquivo(instance)

    @action(detail=True, methods=['get'], url_path='download')
    def download(self, request: Any, uuid: Any=None) -> Any:
        """Retorna o arquivo da exportação. Se o registro tiver conteudo_arquivo e.

        nome_arquivo
        salvos (create recente), devolve esse conteúdo sem chamar APIs.
        Registros antigos
        sem conteúdo continuam funcionando com geração na hora via
        gerar_arquivo.
        """
