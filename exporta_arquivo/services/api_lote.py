...
class ApiLoteCandidatosService:
    ...
    def _fazer_request_get(self, url: str, params: dict, descricao_contexto: str) -> list[dict[str, Any]]:
        ...
        Args:
            ...
            params: Parâmetro params da operação.
            descricao_contexto: Parâmetro descricao contexto da operação.
        ...
        Raises:
            ...
            ExportacaoServiceUnavailableException: Se ocorrer erro nesta operação.
        """
        try:
            ...
            raise ExportacaoServiceUnavailableException(mensagem='Serviço de candidatos indisponível.', detalhes=str(exc)) from exc
        if response.status_code == 404:
            raise ExportacaoNotFoundException(mensagem=f'Dados não encontrados ({descricao_contexto}).', detalhes=f'Parâmetros: {params}')
        if response.status_code >= 500:
            logger.error('API candidatos retornou status %s: %s', response.status_code, response.text[:500])
            raise ExportacaoServiceUnavailableException(mensagem='Serviço de candidatos indisponível.', detalhes=f'Status {response.status_code}')
        if response.status_code != 200:
            raise ExportacaoServiceUnavailableException(mensagem=f'Erro ao obter dados: {descricao_contexto}.', detalhes=f'Status {response.status_code}')
        try:
            data = response.json()
        except ValueError as exc:
            logger.exception('Resposta da API de candidatos não é JSON válido.')
            raise ExportacaoServiceUnavailableException(mensagem='Resposta inválida do serviço de candidatos.', detalhes=str(exc)) from exc
    ...
    def get_candidatos_lote(self, lote_uuid: str) -> list[dict[str, Any]]:
        ...
        Args:
            self: Instância do objeto.
            lote_uuid: Parâmetro lote uuid da operação.
    ...
    def get_candidatos_por_numero_lote(self, concurso_uuid: str, numero_lote: int) -> list[dict[str, Any]]:
        """GET.
        ...
        """
        url = f'{self.base_url}/api/v1/habilitados/'
        params: dict[str, Any] = {'lote__concurso_uuid': str(concurso_uuid), 'numero_lote': numero_lote}
...
class ApiLoteEscolhasService:
    ...
    def get_escolhas_lote(self, candidato_uuids: list[str], concurso_uuid: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise ExportacaoServiceUnavailableException(mensagem='Serviço de escolhas indisponível.', detalhes=str(exc)) from exc
        if response.status_code >= 500:
            logger.error('API escolhas retornou status %s: %s', response.status_code, response.text[:500])
            raise ExportacaoServiceUnavailableException(mensagem='Serviço de escolhas indisponível.', detalhes=f'Status {response.status_code}')
        if response.status_code != 200:
            raise ExportacaoServiceUnavailableException(mensagem='Erro ao obter escolhas do lote.', detalhes=f'Status {response.status_code}')
        try:
            data = response.json()
        except ValueError as exc:
            logger.exception('Resposta da API de escolhas não é JSON válido.')
            raise ExportacaoServiceUnavailableException(mensagem='Resposta inválida do serviço de escolhas.', detalhes=str(exc)) from exc
        if isinstance(data, list):
            return data
        if isinstance(data, dict) and 'results' in data:
            results = data['results']
