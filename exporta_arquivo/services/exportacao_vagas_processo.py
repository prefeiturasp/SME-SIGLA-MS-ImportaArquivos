...
from exporta_arquivo.serializers.exportacao_vagas_sigpec import VagasPayloadSerializer
...
def formatar_arquivo_vagas_processo(
    ...
    """
    Recebe código do cargo (int) e lista de dicts com chaves 'setor' (codigo EOL),
    'vagas_definitivas', 'vagas_precarias'. Retorna a string do arquivo em UTF-8,
    sem cabeçalho, uma linha por registro: codigo_cargo|codigo_EOL|vagas_definitivas|vagas_precarias.
    """
    partes: List[str] = []
    for item in linhas:
...
def buscar_vagas_escolas(
    ...
    O serializer exige codigo_integracao preenchido em cada escola; payload inválido resulta em ValidationError.
    """    
