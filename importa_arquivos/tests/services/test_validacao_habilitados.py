...
from importa_arquivos.services.exceptions import CargoConcursoInvalidoException, ColunaCSVInvalidaException, LayoutNaoConfiguradoException
...
COLUNAS_COM_CARGO = {'email_col': None, 'cpf_col': None, 'dn_col': None, 'cargo_col': 'Codigo_do_Cargo', 'obrigatorias': ['Codigo_do_Cargo', 'Nome']}
...
def test_validar_csv_habilitados_codigo_cargo_vazio_lanca_excecao(layout_com_cargo: Any) -> None:
    ...
    with _mock_concursos_service({10, 20}):
        with pytest.raises(ColunaCSVInvalidaException) as exc:
            validar_csv_habilitados(_csv_bytes(csv_text), importacao_obj=_FakeImportacaoObj())
...
def test_validar_csv_habilitados_api_concursos_indisponivel_lanca_excecao(layout_com_cargo: Any) -> None:
    ...
    with patch('importa_arquivos.services.validacao_habilitados.ApiConcursosService', return_value=mock_service), pytest.raises(CargoConcursoInvalidoException):
        validar_csv_habilitados(_csv_bytes(csv_text), importacao_obj=_FakeImportacaoObj())
