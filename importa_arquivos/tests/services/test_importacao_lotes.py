...
- validar_txt_lotes: arquivo vazio, colunas faltando, linha inválida, sucesso, linhas vazias ignoradas
...
def _linha(lote=1, empresa="EMP01", vaga="VAGA01", identificacao=100,
           chave="CH01", numfunc=999, numvinc=1):
...
class TestValidarLinhaLote:
    ...
    def _row(self, lote="1", empresa="EMP", vaga="VAG", identificacao="100",
             chave="CH", numfunc="999", numvinc="1"):
        return {
            "LOTE": lote, "EMPRESA": empresa, "VAGA": vaga,
            "IDENTIFICACAO": identificacao, "CHAVE_INSCRITO": chave,
            "NUMFUNC": numfunc, "NUMVINC": numvinc,
        }
...
class TestValidarTxtLotes:
    ...
    def test_linha_invalida_levanta_erros_validacao(self):
        # Uma linha válida + uma linha com lote divergente → ErrosValidacaoLotesException
    ...
    def test_leitura_falha_levanta_leitura_csv(self):
        arquivo = MagicMock()
        arquivo.read.side_effect = IOError("disk error")
    ...
    def test_arquivo_com_bom_utf8_sig_decodificado(self):
        ...
        conteudo = HEADER + _linha()
        arquivo = MagicMock()
        # utf-8-sig insere o BOM automaticamente; o serviço usa decode('utf-8-sig') para removê-lo
