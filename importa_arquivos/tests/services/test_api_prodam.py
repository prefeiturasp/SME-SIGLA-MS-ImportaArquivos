...
class TestApiProdamService:
    ...
    def test_init_sem_token_levanta_erro(self, settings):
        ...
        with pytest.raises(ValueError, match='Configurações da API externa não encontradas'):
            ApiProdamService()
    ...
    def test_consultar_resultado_convocacao_ingresso_sucesso(self, settings):
        ...
        with patch('importa_arquivos.services.api_prodam.requests.post') as mock_post:
            mock_resp = Mock()
            mock_resp.text = '{"retorno": "TRUE", "mensagem": "Sucesso", "lstDadosResultadoConvocacaoIngresso": [{"codigoPessoaFisica": "12345678901", "codigoCargo": "123", "descricaoStatus": "ALOCADO"}]}'
            mock_resp.json.side_effect = [resposta_mock, json.dumps(resposta_mock)]
            mock_resp.raise_for_status.return_value = None
            mock_post.return_value = mock_resp
            
            resultado = service.consultar_resultado_convocacao_ingresso(processo_id=123)
    ...
    def test_consultar_resultado_convocacao_ingresso_cria_log(self, settings):
        ...
        with patch('importa_arquivos.services.api_prodam.requests.post') as mock_post:
            mock_resp = Mock()
            mock_resp.text = '{"retorno": "TRUE", "mensagem": "Sucesso", "lstDadosResultadoConvocacaoIngresso": []}'
            mock_resp.json.side_effect = [resposta_mock, json.dumps(resposta_mock)]
            mock_resp.raise_for_status.return_value = None
            mock_post.return_value = mock_resp
    ...
    def test_consultar_resultado_convocacao_ingresso_payload_correto(self, settings):
    ...
    def test_consultar_resultado_convocacao_ingresso_request_exception(self, settings):
        ...
        with patch('importa_arquivos.services.api_prodam.requests.post', side_effect=RequestException('Erro de conexão')):
            with pytest.raises(RequestException, match='Erro ao consultar API externa'):
                service.consultar_resultado_convocacao_ingresso(processo_id=123)

    def test_consultar_resultado_convocacao_ingresso_resposta_invalida(self, settings):
        ...
        with patch('importa_arquivos.services.api_prodam.requests.post') as mock_post:
            ...
            with pytest.raises(ValueError, match='Erro ao processar resposta da API externa'):
                service.consultar_resultado_convocacao_ingresso(processo_id=123)

    def test_consultar_resultado_convocacao_ingresso_resposta_com_erro_validacao(self, settings):
    ...
    def test_consultar_resultado_convocacao_ingresso_nao_quebra_quando_log_falha(self, settings):
        ...
        with patch('importa_arquivos.services.api_prodam.requests.post') as mock_post:
            ...
            with patch('importa_arquivos.services.api_prodam.LogRequestHttp.objects.create', side_effect=Exception('Erro ao criar log')):
                resultado = service.consultar_resultado_convocacao_ingresso(processo_id=123)
                assert resultado['retorno'] == 'TRUE'

    def test_consultar_resultado_convocacao_ingresso_http_error(self, settings):
        ...
        with patch('importa_arquivos.services.api_prodam.requests.post') as mock_post:
            ...
            mock_resp.raise_for_status.side_effect = RequestException('500 Internal Server Error')
            mock_post.return_value = mock_resp
