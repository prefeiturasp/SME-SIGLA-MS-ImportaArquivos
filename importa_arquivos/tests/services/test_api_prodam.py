...
class TestApiProdamService:
    ...
    def test_consultar_resultado_convocacao_ingresso_sucesso(self, settings: Any) -> None:
        ...
        settings.PRODAM_API_SENHA = 'senha'
        service = ApiProdamService()
        resposta_mock = {'retorno': 'TRUE', 'mensagem': 'Sucesso', 'lstDadosResultadoConvocacaoIngresso': [{'codigoPessoaFisica': '12345678901', 'codigoCargo': '123', 'descricaoStatus': 'ALOCADO'}]}
    ...
    def test_consultar_resultado_convocacao_ingresso_cria_log(self, settings: Any) -> None:
        ...
        settings.PRODAM_API_SENHA = 'senha'
        service = ApiProdamService()
        resposta_mock = {'retorno': 'TRUE', 'mensagem': 'Sucesso', 'lstDadosResultadoConvocacaoIngresso': []}
        with patch('importa_arquivos.services.api_prodam.requests.post') as mock_post:
            mock_resp = Mock()
    ...
    def test_consultar_resultado_convocacao_ingresso_resposta_com_erro_validacao(self, settings: Any) -> None:
        ...
        settings.PRODAM_API_SENHA = 'senha'
        service = ApiProdamService()
        resposta_mock = {'retorno': 'TRUE', 'mensagem': 'Sucesso', 'lstDadosResultadoConvocacaoIngresso': [{'codigoPessoaFisica': '12345678901'}]}
        with patch('importa_arquivos.services.api_prodam.requests.post') as mock_post:
            mock_resp = Mock()
            mock_resp.text = '{}'
            mock_resp.json.side_effect = [resposta_mock, json.dumps(resposta_mock)]
