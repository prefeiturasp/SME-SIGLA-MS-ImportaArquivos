...
class TestLogRequestHttpModel:
    ...
    def test_criar_log_request_http_com_todos_campos(self) -> None:
        ...
        metodo = 'GET'
        resposta = '{"data": "test"}'
        log = LogRequestHttp.objects.create(url=url, metodo_http=metodo, processo_id=processo_id, resposta_raw=resposta)
        assert log.url == url
    ...
    def test_log_request_http_ordering(self) -> None:
        ...
        import time
        time.sleep(0.01)
        log2 = LogRequestHttp.objects.create(url='https://api.example.com/endpoint2', metodo_http='GET', resposta_raw='response2')
