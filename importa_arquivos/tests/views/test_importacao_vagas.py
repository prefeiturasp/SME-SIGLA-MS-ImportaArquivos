"""Módulo tests/views/test_importacao_vagas."""
...
def test_importacao_vagas_create_success(api_client: Any, settings: Any) -> None:
    ...
    with patch('importa_arquivos.views.importacao_vagas.validar_csv_vagas') as mock_validar, patch('importa_arquivos.views.importacao_vagas.ApiEscolhasService') as mock_api:
        mock_validar.return_value = ([{'DataFechamentoModulo': '05/09/2025'}], [{'coluna': 'DataFechamentoModulo', 'campo_payload': 'data_fechamento_modulo'}])
        mock_api.return_value.enviar_vagas.return_value = Mock()
        url = reverse('importacao-arquivo-vagas-list')
        resp = api_client.post(url, {'arquivo': arquivo, 'tipo': 'VAGAS'}, format='multipart')
...
def test_importacao_vagas_envio_api_exception(api_client: Any, settings: Any) -> None:
    ...
    with patch('importa_arquivos.views.importacao_vagas.validar_csv_vagas') as mock_validar, patch('importa_arquivos.views.importacao_vagas.ApiEscolhasService') as mock_api:
        mock_validar.return_value = ([{'DataFechamentoModulo': '05/09/2025'}], [{'coluna': 'DataFechamentoModulo', 'campo_payload': 'data_fechamento_modulo'}])
        mock_api.return_value.enviar_vagas.side_effect = ApiEscolhasException(mensagem='Erro externo', detalhes='Detalhes do erro externo', status_code=400, code='ERRO_EXTERNO')
        url = reverse('importacao-arquivo-vagas-list')
        resp = api_client.post(url, {'arquivo': arquivo, 'tipo': 'VAGAS'}, format='multipart')
        assert resp.status_code == 400
...
def test_importacao_vagas_tipo_ue_desabilitado(api_client: Any, settings: Any) -> None:
    ...
    arquivo = SimpleUploadedFile('v.csv', b'DataFechamentoModulo\n05/09/2025\n', content_type='text/csv')
    with patch('importa_arquivos.views.importacao_vagas.validar_csv_vagas') as mock_validar, patch('importa_arquivos.views.importacao_vagas.ApiEscolhasService') as mock_api:
        mock_validar.return_value = ([{'DataFechamentoModulo': '05/09/2025', 'codigo_eol': '123456'}], [{'coluna': 'DataFechamentoModulo', 'campo_payload': 'data_fechamento_modulo'}])
...
class TestImportacaoVagasFiltersAndSearch:
    ...
    def test_filterset_fields_processo_uuid(self, api_client: Any) -> None:
        ...
        ImportacaoArquivoVagas.objects.create(nome_arquivo='arquivo1.csv', arquivo=SimpleUploadedFile('arquivo1.csv', b'content'), status='PENDENTE', processo_uuid=processo_uuid, processo_nome='Processo Teste 1')
        ImportacaoArquivoVagas.objects.create(nome_arquivo='arquivo2.csv', arquivo=SimpleUploadedFile('arquivo2.csv', b'content'), status='PROCESSADO', processo_uuid=str(uuid.uuid4()), processo_nome='Processo Teste 2')
        url = reverse('importacao-arquivo-vagas-list')
    ...
    def test_filterset_fields_processo_nome(self, api_client: Any) -> None:
        ...
        ImportacaoArquivoVagas.objects.create(nome_arquivo='arquivo2.csv', arquivo=SimpleUploadedFile('arquivo2.csv', b'content'), status='PROCESSADO', processo_nome='Processo Teste 2')
        url = reverse('importacao-arquivo-vagas-list')
...
class TestImportacaoVagasConcursoFields:
    ...
    def test_create_with_concurso_fields_in_serializer(self, api_client: Any, settings: Any) -> None:
        ...
        with patch('importa_arquivos.views.importacao_vagas.validar_csv_vagas') as mock_validar, patch('importa_arquivos.views.importacao_vagas.ApiEscolhasService') as mock_api:
            ...
            mock_api.return_value.enviar_vagas.return_value = Mock()
            url = reverse('importacao-arquivo-vagas-list')
            resp = api_client.post(url, {'arquivo': arquivo, 'processo_uuid': processo_uuid, 'processo_nome': 'Processo Teste'}, format='multipart')
            assert resp.status_code in (200, 201)
            mock_api.return_value.enviar_vagas.assert_called_once()
    ...
    def test_create_with_empty_concurso_fields(self, api_client: Any, settings: Any) -> None:
        ...
        with patch('importa_arquivos.views.importacao_vagas.validar_csv_vagas') as mock_validar, patch('importa_arquivos.views.importacao_vagas.ApiEscolhasService') as mock_api:
            ...
            mock_api.return_value.enviar_vagas.return_value = Mock()
            url = reverse('importacao-arquivo-vagas-list')
            resp = api_client.post(url, {'arquivo': arquivo}, format='multipart')
            assert resp.status_code in (200, 201)
            mock_api.return_value.enviar_vagas.assert_called_once()
            call_args = mock_api.return_value.enviar_vagas.call_args
...
class TestImportacaoVagasSerializers:
    ...
    def test_create_serializer_with_concurso_fields(self) -> None:
        ...
        data = {'arquivo': arquivo, 'processo_uuid': processo_uuid, 'processo_nome': 'Processo Teste 2025'}
    ...
    def test_list_serializer_includes_processo_fields(self, api_client: Any) -> None:
        ...
        from importa_arquivos.models import ImportacaoArquivoVagas
        processo_uuid = str(uuid.uuid4())
        obj = ImportacaoArquivoVagas.objects.create(nome_arquivo='teste.csv', arquivo=SimpleUploadedFile('teste.csv', b'content'), status='PENDENTE', processo_uuid=processo_uuid, processo_nome='Processo Teste 2025')
        url = reverse('importacao-arquivo-vagas-detail', kwargs={'pk': obj.pk})
        resp = api_client.get(url)
        assert resp.status_code == 200
    ...
    def test_list_serializer_with_null_processo_fields(self, api_client: Any) -> None:
        ...
        from django.core.files.uploadedfile import SimpleUploadedFile
        from importa_arquivos.models import ImportacaoArquivoVagas
        obj = ImportacaoArquivoVagas.objects.create(nome_arquivo='teste.csv', arquivo=SimpleUploadedFile('teste.csv', b'content'), status='PENDENTE', processo_uuid=None, processo_nome=None)
        url = reverse('importacao-arquivo-vagas-detail', kwargs={'pk': obj.pk})
        resp = api_client.get(url)
        assert resp.status_code == 200
...
def test_download_erros_filtra_por_importacao_uuid(api_client: Any) -> None:
    ...
    importacao2 = ImportacaoArquivoVagas.objects.create(nome_arquivo='v2.csv', arquivo=arquivo2, tipo='VAGAS')
    content_type = ContentType.objects.get_for_model(ImportacaoArquivoVagas)
