...
- create: sucesso (201), ErrosValidacaoLotesException (400), BaseImportacaoException (400),
          exceção genérica (400), falha na API de candidatos (400), serializer inválido (400)
...
def _registros_validos():
    return [{"lote": 1, "empresa": "EMP01", "vaga": "VAGA01",
              "identificacao": 100, "chave_inscrito": "CH01",
              "numfunc": 999, "numvinc": 1}]
...
class TestImportacaoLotesCreate:
    ...
    def test_sucesso_retorna_201_e_status_concluido(self, api_client):
        with patch("importa_arquivos.views.importacao_lotes.validar_txt_lotes",
                   return_value=_registros_validos()), \
             patch("importa_arquivos.views.importacao_lotes.ApiCandidatosService") as MockApi:
    ...
    def test_api_candidatos_falha_retorna_400(self, api_client):
        with patch("importa_arquivos.views.importacao_lotes.validar_txt_lotes",
                   return_value=_registros_validos()), \
             patch("importa_arquivos.views.importacao_lotes.ApiCandidatosService") as MockApi:
            MockApi.return_value.salvar_lotes.side_effect = ImportacaoServiceUnavailableException(
    ...
    def test_serializer_invalido_sem_concurso_uuid_retorna_400(self, api_client):
    ...
    def test_serializer_invalido_sem_arquivo_retorna_400(self, api_client):
        resp = api_client.post(LOTES_LIST_URL,
                               {"concurso_uuid": str(uuid.uuid4())}, format="multipart")
        assert resp.status_code == 400

    def test_salvar_lotes_chamado_com_concurso_uuid_e_registros(self, api_client):
        ...
        with patch("importa_arquivos.views.importacao_lotes.validar_txt_lotes",
            ...
            api_client.post(LOTES_LIST_URL, _payload(concurso_uuid=concurso_uuid),
                            format="multipart")
...
class TestImportacaoLotesList:
    ...
    def test_filtro_por_status(self, api_client):
        ImportacaoLotes.objects.create(
            nome_arquivo="a.txt", arquivo=SimpleUploadedFile("a.txt", b"x"),
            tipo="LOTES", status="CONCLUIDO", concurso_uuid=uuid.uuid4(),
        )
        ImportacaoLotes.objects.create(
            nome_arquivo="b.txt", arquivo=SimpleUploadedFile("b.txt", b"x"),
            tipo="LOTES", status="ERRO", concurso_uuid=uuid.uuid4(),
        )
...
class TestImportacaoLotesRetrieve:
    ...
    def test_retrieve_existente_retorna_200(self, api_client):
        reg = ImportacaoLotes.objects.create(
            nome_arquivo="x.txt", arquivo=SimpleUploadedFile("x.txt", b"x"),
            tipo="LOTES", status="CONCLUIDO", concurso_uuid=uuid.uuid4(),
        )
