...
class ApiEscolhasService:
    ...
    def enviar_vagas(self, registros: list[dict[str, Any]], estrutura: list[dict[str, Any]], processo_uuid: str='', processo_nome: str='', headers: dict[str, str] | None=None, importacao_obj: Any | None=None) -> dict:
        ...
        payload = {'vagas': dados, 'processo_uuid': processo_uuid, 'processo_nome': processo_nome}
        try:
            ...
            if response.status_code == 400:
                ...
                if isinstance(data, dict) and data.get('code') == 'TIPO_UE_DESABILITADO':
                    raise TipoUEDesabilitadoException(mensagem=str(data.get('detail') or 'Tipo de UE desabilitado'), detalhes='TIPO_UE_DESABILITADO')
            if response.status_code >= 400:
                raise ApiEscolhasException(mensagem='Falha ao enviar vagas para API externa', detalhes=response.text or f'Status {response.status_code}', status_code=response.status_code)
    ...
    def _transformar_escolhas_prodam_para_escolhas(self, dados_prodam: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in dados_prodam:
            escolha = {'cpf': item.get('codigoPessoaFisica', ''), 'codigo_cargo': item.get('codigoCargo', ''), 'codigo_eol': item.get('codigoUnidadeAlocacao') or '', 'tipo_vaga': item.get('tipoVaga') or '', 'situacao': 'ESCOLHA'}
    ...
    def enviar_escolhas_prodam(self, processo_uuid: str, concurso_uuid: str, dados_prodam: list[dict[str, Any]], headers: dict[str, str] | None=None, importacao_obj: Any | None=None) -> dict:
        ...
        payload = {'processo_uuid': str(processo_uuid), 'concurso_uuid': str(concurso_uuid), 'escolhas': escolhas}
        try:
            ...
            raise
        if response.status_code >= 400:
            raise ApiEscolhasException(mensagem='Falha ao enviar escolhas para API externa', detalhes=response.text or f'Status {response.status_code}', status_code=response.status_code)
