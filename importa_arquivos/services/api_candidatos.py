...
from importa_arquivos.services.exceptions import ApiCandidatosException, ImportacaoBadRequestException, ImportacaoServiceUnavailableException
...
class ApiCandidatosService:
    ...
    def enviar_habilitados(self, registros: list[dict[str, Any]], estrutura: list[dict[str, Any]], concurso_uuid: str, concurso_nome: str, headers: dict[str, str] | None=None, importacao_obj: Any | None=None) -> Response:
        ...
        payload = {'concurso_uuid': concurso_uuid, 'concurso_nome': concurso_nome, 'candidatos': dados_transformados}
        try:
        ...
        if response.status_code >= 400:
            raise ApiCandidatosException(mensagem='Falha ao enviar candidatos para API externa', detalhes=response.text or f'Status {response.status_code}', status_code=response.status_code)
    ...
    def salvar_lotes(self, concurso_uuid: str, lotes: list, importacao_obj: Any=None) -> int:
        ...
        try:
            ...
            raise ImportacaoServiceUnavailableException(mensagem='Serviço de candidatos (salvar-lotes) indisponível.', detalhes=str(exc)) from exc
        if response.status_code == 400:
        ...
        if response.status_code >= 500:
            logger.error('API salvar-lotes retornou status %s: %s', response.status_code, response.text)
            raise ImportacaoServiceUnavailableException(mensagem='Serviço de candidatos (salvar-lotes) indisponível.', detalhes=f'Status {response.status_code}: {response.text}')
        if response.status_code not in (200, 201):
            logger.error('API salvar-lotes retornou status %s: %s', response.status_code, response.text)
            raise ImportacaoServiceUnavailableException(mensagem='Erro ao salvar lotes.', detalhes=f'Status {response.status_code}: {response.text}')
