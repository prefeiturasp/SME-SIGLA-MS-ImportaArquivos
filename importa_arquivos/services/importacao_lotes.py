...
def _validar_linha_lote(row: dict, num_linha: int, lote_referencia: int | None) -> tuple[LinhaLoteSIGPEC | None, str, int | None]:
    ...
    try:
        obj = LinhaLoteSIGPEC(lote=row.get('LOTE', '').strip(), empresa=row.get('EMPRESA', '').strip(), vaga=row.get('VAGA', '').strip(), identificacao=row.get('IDENTIFICACAO', '').strip(), chave_inscrito=row.get('CHAVE_INSCRITO', '').strip(), numfunc=row.get('NUMFUNC', '').strip(), numvinc=row.get('NUMVINC', '').strip() or None)
    except ValidationError as exc:
        erros = '\n'.join([f"Linha: {num_linha} | Identificacao: {identificacao} - Campo '{e['loc'][0]}': {e['msg']}" for e in exc.errors()])
...
def validar_txt_lotes(arquivo: Any, importacao_obj: Any=None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise LeituraCSVException(mensagem='Nao foi possivel ler o arquivo de lotes.', detalhes=f'Detalhes tecnicos: {exc}') from exc
    if not text or not text.strip():
        raise ArquivoLotesVazioException(mensagem='O arquivo de lotes esta vazio.', detalhes='Arquivo sem conteudo util para processamento.')
    reader = csv.DictReader(io.StringIO(text), delimiter=';')
    headers_csv = {h for h in reader.fieldnames or [] if h}
    colunas_faltando = COLUNAS_ESPERADAS - headers_csv
    if colunas_faltando:
        raise ColunaCSVInvalidaException(mensagem='Cabecalho invalido para importacao de lotes.', detalhes=f'Colunas ausentes: {sorted(colunas_faltando)}. Esperado: {sorted(COLUNAS_ESPERADAS)}')
    ...
    for row in reader:
        ...
        if not valores:
            continue
        obj, erro, lote_referencia = _validar_linha_lote(row, reader.line_num, lote_referencia)
        if erro:
            erros.append(erro)
        else:
            registros.append(obj.model_dump())  # type: ignore[union-attr]
    if not registros:
        raise ArquivoLotesVazioException(mensagem='O arquivo nao contem registros validos.', detalhes='Apenas cabecalho ou linhas vazias foram encontradas.')
    if erros:
        raise ErrosValidacaoLotesException(mensagem='Erro ao validar os dados do arquivo.', detalhes='\n'.join(erros))
