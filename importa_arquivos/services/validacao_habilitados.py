...
def _resolver_colunas(estrutura: list[dict]) -> dict:
    ...
    for item in estrutura:
        ...
        if email_col is None:  # noqa: SIM102
            if campo == "email" or (
        ...
        if cpf_col is None:  # noqa: SIM102
            if campo == "cpf" or (
                isinstance(coluna, str) and coluna.lower() == "cpf"
            ):
                cpf_col = coluna
        ...
        if cargo_col is None:  # noqa: SIM102
...
def _validar_linhas(
    ...
    for idx, row in enumerate(registros, start=2):
        ...
        if email_col:
            ...
            if valor_email is not None and str(valor_email).strip() != "":  # noqa: SIM102
...
def validar_csv_habilitados(
    ...
    try:
        ...
        raise LayoutNaoConfiguradoException(  # noqa: B904
    ...
    if headers_csv != colunas_esperadas:
        ...
        detalhes = f"Encontradas: {sorted(headers_csv)} | Esperadas: {sorted(colunas_esperadas)}"  # noqa: E501
