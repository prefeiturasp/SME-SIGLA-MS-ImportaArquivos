    ...
    for item in estrutura:
        ...
        if cpf_col is None:
            if campo == 'cpf' or (isinstance(coluna, str) and coluna.lower() == 'cpf'):
                cpf_col = coluna
        if dn_col is None and (campo == 'data_nascimento' or (isinstance(coluna, str) and coluna.lower() in ('datanascimento',))):
            dn_col = coluna
        if cargo_col is None:
            if campo == 'codigo_cargo' or coluna == 'Codigo_do_Cargo':
                cargo_col = coluna
    return {'email_col': email_col, 'cpf_col': cpf_col, 'dn_col': dn_col, 'cargo_col': cargo_col, 'obrigatorias': obrigatorias}
...
def _validar_linhas(colunas: dict, registros: list[dict]) -> dict[int, list[str]]:
    ...
    for idx, row in enumerate(registros, start=2):
        erros: list[str] = []
        faltantes = [col for col in obrigatorias if row.get(col) is None or str(row.get(col)).strip() == '']
        if faltantes:
...
def _validar_email_duplicado_por_cpf(colunas: dict, registros: list[dict]) -> dict[int, list[str]]:
    ...
    for idx, row in enumerate(registros, start=2):
        ...
        if cpf_norm != expected:
            erros_por_linha.setdefault(idx, []).append(f'Email duplicado. CPF {cpf_norm} com o mesmo email que o CPF {expected}')
    return erros_por_linha
...
def _validar_codigo_cargo(colunas: dict, registros: list[dict], codigos_cargo_concurso: set) -> dict[int, list[str]]:
    ...
    for idx, row in enumerate(registros, start=2):
        ...
        if valor is None or str(valor).strip() == '':
            erros_por_linha.setdefault(idx, []).append('Codigo_do_Cargo não pode estar em branco')
            continue
        try:
            codigo_int = int(str(valor).strip())
        except (ValueError, TypeError):
            erros_por_linha.setdefault(idx, []).append(f"Codigo_do_Cargo deve ser um número inteiro válido -> '{valor}'")
            continue
        if codigo_int not in codigos_cargo_concurso:
            codigos_validos = ', '.join((str(c) for c in sorted(codigos_cargo_concurso)))
            erros_por_linha.setdefault(idx, []).append(f"Codigo_do_Cargo '{valor}' não possui relação com o concurso selecionado. Códigos válidos: {codigos_validos}")
    return erros_por_linha
...
def validar_csv_habilitados(arquivo: Any, importacao_obj: Any=None) -> tuple[list[dict], list[dict]]:
    ...
    if headers_csv != colunas_esperadas:
        ...
        detalhes = f'Encontradas: {sorted(headers_csv)} | Esperadas: {sorted(colunas_esperadas)}'  # type: ignore[type-var]
    ...
    if concurso_uuid:
        service = ApiConcursosService(base_url=settings.CONCURSOS_API_URL, timeout_seconds=getattr(settings, 'CONCURSOS_API_TIMEOUT', 10))
