...
class ApiProdamService:
    ...
    def consultar_resultado_convocacao_ingresso(self, processo_id: int) -> dict[str, Any]:
        ...
        payload = {'usuario': self._usuario, 'senha': self._senha, 'identificadorChamadaSistema': processo_id}
        response = None
        try:
            logger.info(f'Consultando API externa: processo_id={processo_id}')
            response = requests.post(url, json=payload, headers=headers, timeout=self.timeout_seconds)  # type: ignore[arg-type]
            if response is not None:
                try:
                    LogRequestHttp.objects.create(url=url, metodo_http='POST', processo_id=processo_id, resposta_raw=response.text)  # type: ignore[misc]
            ...
            logger.info(f'Resposta API externa recebida: retorno={data.get('retorno')}, registros={len(data.get('lstDadosResultadoConvocacaoIngresso', []))}')
            return serializer.validated_data  # type: ignore[no-any-return]
        except RequestException as exc:
            logger.error(f'Erro ao consultar API externa: {exc}')
            raise RequestException(f'Erro ao consultar API externa: {exc}') from exc
        except Exception as exc:
            logger.error(f'Erro inesperado ao processar resposta da API externa: {exc}')
            raise ValueError(f'Erro ao processar resposta da API externa: {exc}') from exc
