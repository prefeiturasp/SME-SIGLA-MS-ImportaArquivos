...
from ..serializers import ImportacaoErrosListSerializer, ImportacaoEscolhasCreateSerializer, ImportacaoEscolhasListSerializer, queryset_erros_por_modelo
...
class ImportacaoEscolhasViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        instance = ImportacaoEscolhas.objects.create(processo_uuid=processo_uuid, processo_id=processo_id, concurso_uuid=concurso_uuid, status='PROCESSANDO')
        try:
            logger.info(f'Consultando API externa: processo_id={processo_id}')
            resposta_api = ApiProdamService().consultar_resultado_convocacao_ingresso(processo_id=processo_id)
            if resposta_api.get('retorno') != 'TRUE':
                ...
                instance.save()
                registrar_erro(instance, mensagem='Erro na resposta da API PRODAM', detalhes=mensagem_erro)
                return Response({'detail': f'Erro na API PRODAM: {mensagem_erro}'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            api_escolhas_service = ApiEscolhasService(base_url=settings.ESCOLHA_API_URL, timeout_seconds=settings.ESCOLHA_API_TIMEOUT)
            api_escolhas_service.enviar_escolhas_prodam(processo_uuid=processo_uuid, concurso_uuid=concurso_uuid, dados_prodam=dados_prodam, importacao_obj=instance)
            instance.status = 'CONCLUIDO'
            instance.save()
            logger.info(f'Importação concluída com sucesso: {len(dados_prodam)} registros')
        except ApiEscolhasException as exc:
            logger.error(f'Erro da API de escolhas durante importação: {exc}', exc_info=True)
            instance.status = 'ERRO'
            instance.save()
            with contextlib.suppress(Exception):
                registrar_erro(instance, mensagem='Erro durante importação de escolhas', detalhes=exc.detalhes or str(exc), exc=exc)
            resposta = {'detail': exc.mensagem or 'Falha ao processar importação de escolhas', 'detalhes': exc.detalhes or str(exc)}
            if exc.code:
                resposta['code'] = exc.code
            return Response(resposta, status=exc.status_code or status.HTTP_400_BAD_REQUEST)
        except RequestException as exc:
            logger.error(f'Erro de request durante importação de escolhas: {exc}', exc_info=True)
            instance.status = 'ERRO'
            instance.save()
            with contextlib.suppress(Exception):
                registrar_erro(instance, mensagem='Erro durante importação de escolhas', detalhes=str(exc), exc=exc)
            return Response({'detail': f'Erro ao processar importação: {str(exc)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
