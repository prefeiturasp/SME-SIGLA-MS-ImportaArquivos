...
from ..serializers import ImportacaoArquivoVagasCreateSerializer, ImportacaoArquivoVagasListSerializer, ImportacaoErrosListSerializer, queryset_erros_por_modelo
from ..services.api_escolhas import ApiEscolhasService
from ..services.exceptions import ApiEscolhasException, ColunaCSVInvalidaException, LayoutNaoConfiguradoException, LeituraCSVException, TipoUEDesabilitadoException
...
class ImportacaoArquivoVagasViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        serializer.validated_data.get('processo_uuid') or request.data.get('processo_uuid')
        serializer.validated_data.get('processo_nome') or request.data.get('processo_nome')
        try:
        ...
        try:
            ApiEscolhasService(base_url=settings.ESCOLHA_API_URL).enviar_vagas(registros=registros, estrutura=estrutura, processo_uuid=str(instance.processo_uuid) if instance.processo_uuid else '', processo_nome=str(instance.processo_nome) if instance.processo_nome else '', importacao_obj=instance)
